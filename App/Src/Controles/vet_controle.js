...
const perfilVet = async (req, res) => {
    ...
    const [vet, examesVet] = await Promise.all([
        Promise.resolve(modeloVet.Veterinario.findOne({where: {nome: nome, prontuario: prontuario, senha: senha}}, { raw: true })),
...
const criarVet = async (req, res) => {
    const novoVet = await Promise.resolve(modeloVet.criarVet(req.body));
